...
- `skeeks/cms-backend/src/widgets/BackendModelHeader.php`;
- `skeeks/cms-backend/src/grid/BackendEntityLinkColumn.php`;
- `skeeks/cms-backend/src/grid/DefaultActionColumn.php`;
...
The shared `BackendSectionHeader` renders the page title, description and
actions. Prefer semantic `BackendIcon` names while preserving legacy icon-class
compatibility. The first resolved action is primary and later actions are
secondary unless their variant is explicit. Referencing an action through
`backendAction` must preserve that action's visibility, access and standard
controller-action interaction contract.
...
Treat `--sx-collection-row-*` as the canonical collection-row token family.
Historical `--sx-collection-item-*`, list and backend-grid row variables may
point one-way to it during migration; canonical collection and shell CSS must
not route back through those aliases. Primary entities remain neutral,
supporting copy uses secondary/subtle roles, money uses
`sx-collection-cell__amount`, state uses semantic status/text roles and linked
records use `sx-preview-card__related`. Use `sx-preview-card-column` when a
compact relation column needs stable width instead of inline cell geometry.
...
## Model cards and entity links

Use `BackendModelHeader` as the canonical renderer for new and migrated model
headers. It owns the semantic hierarchy, optional image, conventional
ID/date/author metadata, responsive side composition, safe back link and the
standard destructive action. Domain `_model_header.php` files provide plain
title text and deliberately trusted `titleSuffix`, media, `metaItems`, status,
toolbar, actions or side slots; they must not copy the outer header or
Bootstrap grid.
When one domain repeats the same metadata policy, create a thin domain adapter
instead of duplicating the renderer or forcing unrelated models through it.

Use `BackendQuickAccessFavoriteButton` for quick-access favorites. Shared
backend code owns markup, active state and interaction; domain code supplies
only the serializable entity payload. Keep destructive actions in the model
...
`urlParams`, `linkOptions`, `content` and `tag` only for real domain
differences. `DefaultActionColumn` remains a compatibility facade: it derives
the owning controller route from the grid view context and that controller's
model primary-key attribute, then delegates all rendering to
`BackendEntityLinkColumn`. New or deliberately migrated grids should prefer
...
or hidden action proxies in the DOM.
