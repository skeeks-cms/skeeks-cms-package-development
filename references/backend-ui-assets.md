...
- `BackendIframeAsset` owns product-neutral same-origin iframe communication;
  Admin and UPA register it directly, while `UnifyAdminIframeAsset` may remain
  only as a thin legacy alias without the Unify administration runtime;
...
`BackendComponent` owns creation and installation of a configured backend
theme and exposes a protected configuration hook. Keep its base `themeClass`
nullable so legacy backends remain untouched. Standard Admin and UPA
components opt in and own their scope, palette, header mode and lazy
customizer defaults; projects normally replace only `themeClass` instead of
repeating the assembly in `beforeRun` handlers.
...
their guest asset, so neither its Composer graph nor its standard path map may
depend on `cms-theme-unify-v2`. Keep Unify only as an explicitly selected
...
a compatibility alias. Backend shop product and collection galleries use
`ShopAdminGallery` with the page-specific `AdminShopGalleryAsset`. Do not load
Slick, `UnifyThemeStickAsset` or the frontend Unify theme merely to make a
gallery sticky; full-image Fancybox remains its own conditional component.
...
settings cascade for user, site and defaults. Only administrators may change a
...
missing values fall back to the shared dark header and must never reach CSS as
arbitrary values.

Keep the light/dark switcher in the normal shell payload, but lazy-load the
customizer drawer markup, CSS and JavaScript only when the user opens it.
Browser preview derivation must remain in parity with `BackendThemePalette`;
closing without saving removes preview CSS. Never expose raw `--sx-*` names as
editor fields.

Interpret `logoSrcLight` and `logoSrcDark` as logo variants for light and dark
header backgrounds, not page themes. Do not infer logo colors or apply
brightness/invert filters to uploaded artwork. `CmsSite::image_id` is the dark
header variant and universal fallback; `CmsSite::logo_light_image_id` is the
optional light-header variant. Resolve these once through
`AdminSettingsComponent::applyLogoSources()` for admin, UPA and future
cabinets.

## Toast notifications

`sx.notify.*` is the single public toast API for shared packages, Admin, UPA
and future cabinets. Preserve `defaul`, `notice`, `info`, `success`, `warning`,
`error` and `fail`; use `sx.notify.show(text, {type: ...})` when the role is
dynamic. The dependency-free runtime is
`skeeks\sx\assets\ComponentNotifyToast`; backend semantic mapping belongs to
`BackendNotifyAsset`. Do not add jGrowl, Toastr or a project-local toast
runtime to the standard asset graph.

The shared API may configure `duration`, `position`, `title`, `sticky`,
`closable`, `pauseOnHover`, `showProgress`, `maxVisible`, `actions`, `id`/`key`,
`image`, `onClick` and `onClose`; retain `life` and `header` aliases for old
callers. Use `allowHtml: false` for untrusted or user-provided content. Toast
surfaces, focus, offsets and state colors consume backend semantic variables.
Verify the legacy methods and extended API in Admin and UPA, both explicit
themes, mobile width, keyboard close, reduced motion and the absence of
`jquery.jgrowl.*` in the standard graph.

## Container blocking and loading

`sx.block(target, options)` and `new sx.classes.Blocker(target, options)` are
the single public contract for blocking a container, form or page. Keep the
returned instance and call `.unblock()`; call `.update(content, options)` when
the status changes during the same operation. The dependency-free runtime
belongs to the standard `skeeks/yii2-sx` `Custom` asset, while semantic theme
mapping belongs to `BackendBlockerAsset`. Do not add jQuery BlockUI, local
overlays or package-specific loading animations to standard Admin or UPA.

Supported options include `delay`, `minDuration`, `showSpinner`, `shimmer`,
`text`, the legacy `message` alias, `allowHtml`, `className`, `ariaLabel`,
`onBlock` and `onUnblock`. Calls on the same target are reference-counted, so
each successful `block()` requires a matching `unblock()`. Shared code owns
`aria-busy`, compact-container behavior, delayed display, minimum visible
duration and temporary-position restoration.

Backend action drawers use this blocker from insertion until the meaningful
iframe load. Ignore the initial `about:blank`, unblock on close and use
idempotent namespaced trigger bindings. The shared window stack must coalesce
near-simultaneous opens of the same URL. Guest/auth preloader markup may remain
structurally separate before the application runtime starts, but it consumes
the same variables, geometry, theme values and reduced-motion behavior.

Verify blocker changes on a compact control, a large container and a
full-page/auth flow, including natural Ajax/PJAX completion, repeated and
nested calls, both themes and reduced motion. The standard graph must not load
`jquery.blockUI`.
...
- Treat `skeeks/cms-backend/ASSET_BUDGET.md` as the measured request ceiling
  for the empty layout, Admin table/form/model card, UPA table and heavy file
  manager. A lower verified count becomes the new ceiling. A higher count must
  identify its conditional owner and prove the asset remains absent on pages
  that do not render that component; bundling unrelated files does not satisfy
  the budget.
