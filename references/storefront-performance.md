...
Точечные кеши сайта по ID, основного адреса и shop_site описаны в AGENTS.md
пакетов cms и cms-shop. Это явные opt-in в конкретных методах, не глобальное
кеширование ActiveRecord. Только в web генератор URL переиспользует совпадающий
по ID cms.currentTree. Не добавлять найденные при разборе URL модели в статическую
карту: долгоживущие консольные процессы будут удерживать разделы и их связи.

Для фильтров, которым нужны только идентификаторы складов, cms-shop предоставляет
getSupplierStoreIds; контракт setters и проектных переопределений описан в его
AGENTS.md. SQL column() возвращает значения без приведения типов ActiveRecord:
сохранять тип через ColumnSchema::phpTypecast. Снижение гидратации проверять
отдельно от числа SQL: облегчённый SELECT может уменьшить память при том же
количестве запросов. Не обещать такой же эффект страницам, где модели уже
нужны другим потребителям.
